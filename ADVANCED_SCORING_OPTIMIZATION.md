...
3. 🐛 **對話輪次計算錯誤**（關鍵Bug）⭐ **已修復**
   - 問題：AI客戶的自動開場白被計為第1輪對話
   - 用戶反饋：「首輪由客戶所開展的話語，不計算為首輪對話，首輪對話應該是用戶輸入的第一句說話」
   - 影響：輪次錯位，導致評分標準不準確（例：用戶輸入1次被當作2輪對話）
...
✅ **關鍵Bug修復**: 修正對話輪次計算邏輯（計算用戶發言而非AI發言）  
...
### 🐛 **關鍵Bug修復：對話輪次計算邏輯**（v2.3.1）⭐ **最新修復**

**問題根源**：
```javascript
// ❌ 舊邏輯（錯誤）
const therapistMessages = conversation.filter((msg) => msg.role === 'assistant')
const conversationRounds = therapistMessages.length
```
- 計算的是 AI 客戶的發言次數（`role='assistant'`）
- AI 客戶的自動開場白（「你好，請問有什麼可以幫到你？」）被計為第 1 輪
- 用戶（治療師）的第一句話實際上是第 2 輪對話
- **結果**：輪次錯位，評分標準不準確
...
```
- 計算用戶（治療師）的發言次數（`role='user'`）
- AI 客戶的自動開場白**不計入輪次**（僅作為初始化）
- 用戶的第一句話才是真正的第 1 輪對話
- **結果**：輪次正確，評分標準準確

**修正影響**：
| 用戶輸入次數 | 舊邏輯（錯誤） | 新邏輯（正確） | 評分標準 |
|-------------|---------------|---------------|---------|
| 0 次 | 1 輪（AI開場白） | **0 輪** | **4 分** |
| 1 次 | 2 輪 | **1 輪** | **~7 分** |
| 2 次 | 3 輪 | **2 輪** | **≤30 分** |
| 3 次 | 4 輪 | **3 輪** | **≤45 分** |

**用戶反饋原文**：
> 「首輪由客戶所開展的話語，不計算為首輪對話，首輪對話應該是用戶輸入的第一句說話」
